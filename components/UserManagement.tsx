...
const UserManagement: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
      ...
      if (currentUser?.role === 'supervisor' && formData.role === 'admin') return alert("عفواً، لا يمكنك إنشاء حساب مدير.");
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-in zoom-in-95">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" required placeholder="الاسم" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="email" required placeholder="Email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="text" required placeholder="Username" disabled={!!formData.id} value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="password" placeholder="Password" value={formData.password || ''} onChange={e => setFormData({...formData, password: e.target.value})} className="p-3 border rounded-lg" />
                    <select value={formData.role} onChange={(e) => handleRoleChange(e.target.value)} disabled={currentUser.role === 'delegate'} className="p-3 border rounded-lg">
                        {currentUser.role === 'admin' && <option value="admin">Admin</option>}
                        <option value="supervisor">Supervisor</option>
                        <option value="delegate">Delegate</option>
                    </select>
                    {(formData.role === 'delegate' || formData.role === 'supervisor') && (
                        <select value={formData.supervisorId || ''} onChange={(e) => setFormData({...formData, supervisorId: Number(e.target.value)})} disabled={currentUser.role === 'supervisor'} className="p-3 border rounded-lg">
                ...
                {/* PERMISSION MATRIX */}
                <div>
                    <h4 className="font-bold text-sm text-gray-500 mb-2 uppercase">صلاحيات الوصول</h4>
                    <div className="border rounded-xl overflow-hidden text-sm">
                         <table className="w-full text-center">
                             <thead className="bg-gray-100 text-xs">
                                 <tr><th className="p-2 text-right">Screen</th>{ACTIONS.map(a => <th key={a.id} className="p-2">{a.label}</th>)}</tr>
                             </thead>
                             <tbody>
                                 {RESOURCES.map(res => (
                                     <tr key={res.id} className="border-t border-gray-50">
                                         <td className="p-2 text-right font-bold text-gray-600 bg-gray-50">{res.label}</td>
                                         {ACTIONS.map(act => {
                                             ...
                                             return (
                                                 <td key={act.id} className="p-2 border-l border-gray-50">
                                                     <input type="checkbox" checked={!!checked} disabled={disabled} onChange={() => togglePermission(res.id, act.id)} className="w-4 h-4 accent-green-600 cursor-pointer disabled:opacity-30" />
                                                 </td>
                                             )
                                         })}
                                     </tr>
            ...
            <div className="p-4 border-t flex justify-end gap-3 bg-gray-50">
                <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-lg font-bold text-gray-600">إلغاء</button>
                <button onClick={handleSave} className="px-6 py-2 rounded-lg font-bold text-white bg-primary">حفظ</button>
            </div>
          </div>
        </div>
      )}
